...
reset:
    ...
    lda #MON_PIN
    tsb VIA_DDRA 
    stz NUMBER ; initialize the number to 0

 loop:
    jsr lcd_clear
    lda #text & $ff      ; low address byte
    sta LCD_STR_BASE        ; store low byte first
    lda #text>>8         ; high address byte
...
text: .asciiz "n = "
n=2222
delay_val: .byte n&$ff, (n>>8)&$ff, (n>>16)&$ff, (n>>24)&$ff
...
delay0: ; load the variable with the delay value
    lda delay_val,Y     ; abs indexed x (4 cycles)
